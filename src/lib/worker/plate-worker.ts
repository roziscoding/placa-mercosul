...
import { BoxGeometry, BufferGeometry, BufferAttribute, Mesh, MeshStandardMaterial, Group, Vector3 } from "three";
  ...
  try {
    ...
    if (msg.type === "build") {
      ...
      for (const m of meshes) {
        transferables.push(m.geometry.position.buffer);
        transferables.push(m.geometry.normal.buffer);
        if (m.geometry.index) transferables.push(m.geometry.index.buffer);
      }
